        ...
        if (response.status != 200) {
            ...
            for (const [resKey, resValue] of Object.entries(resolutions)) {
                ...
                if (result) {
                    ...
                    fakePlaylist += `#EXT-X-MEDIA:TYPE=VIDEO,GROUP-ID="${quality}",NAME="${quality}",AUTOSELECT=${enabled},DEFAULT=${enabled}`;
                    fakePlaylist += `#EXT-X-STREAM-INF:BANDWIDTH=${startQuality},CODECS="${result.codec},mp4a.40.2",RESOLUTION=${resValue.res},VIDEO="${quality}",FRAME-RATE=${fps}`;
                    fakePlaylist += url;
                    startQuality -= 100;
                };
            };
